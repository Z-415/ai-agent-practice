...
>
> 📝 **卡住了 → 让 AI 助手把这条追加到 `问题笔记.md`**
> （文件里第一节就是「给 AI 助手的指令」，把它指给你的 AI 助手看即可）
...
| 不知道 `sorted(key=)` 怎么用 | **让 AI 助手把这条追加到 `问题笔记.md`**，同时把 C 结构体类比表再看一遍 |
| 超过 40 分钟没进展 | **立刻问**，别死磕；问完顺手记进 `问题笔记.md` |
| 算法题 25 分钟没思路 | **看题解**，关掉题解后**从空白重写一遍**（这是允许的，抄完就走不算） |

> 📝 **规矩**：每次卡住 → 立刻记进 `问题笔记.md`，**不要攒**。
> 攒到周末你就忘了当时卡在哪，记下来的是"你以为你卡在哪"。
> 「复述」那一栏必须**你自己写** —— 那是这个文件唯一的价值。（AI 只写答案，不写复述）
